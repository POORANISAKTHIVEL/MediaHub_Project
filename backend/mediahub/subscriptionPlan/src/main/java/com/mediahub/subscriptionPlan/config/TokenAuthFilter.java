        ...
        if (auth != null && auth.startsWith("Bearer ")) {
            ...
            if (!token.isBlank()) {
                try {
                    ...
                    if (roleType != null) {
                        authorities.add(new org.springframework.security.core.authority.SimpleGrantedAuthority("ROLE_" + roleType.toUpperCase()));
                    }
                    Object perms = claims.get("permissions");
                    if (perms instanceof java.util.Collection) {
                        for (Object p : (java.util.Collection<?>) perms) {
                            if (p != null) authorities.add(new org.springframework.security.core.authority.SimpleGrantedAuthority(p.toString()));
                        }
                    }
